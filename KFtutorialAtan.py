...
def EstimateState(KFparam, measure):
    ''' this function update the Kalman filter parameter including the prediction of the mean beam current
    and the standard deviation with the new measure
    KFparam is a KFpar type object containing the vectors for the Beam current estimate and the stability
    measure is a scalar containing the instant measure of the beam current'''
...
thresh = 8E-1  # threshold on the slope set for the graphic purposes
cLin = 10E-2
cMeas = 10E+2

# Generating the Signal

N = 1000 # setting the time span
time = np.linspace(0, 100, N) # setting the time vector
pos = np.arctan(time/1 - 50)*30 # creating noiseless data
sig = 5 # setting noise standard deviation
mes = pos + np.random.randn(len(pos)) * sig # creating the measure vector (adding noise)

Pos = np.random.randint(300, size = 10) + 700 # adding peaks to challenge the system
mes[Pos] += 50

# initializing KF
KF = KFpar(cLin = cLin, cMeas = cMeas)  # Confidence in lin dynamic, confidence in measurement
...
for k in range(len(time) - 1) :
    ...
    KF = EstimateState(KF, mes[k + 1])  # Update of the Kalman estimate with the current measure
